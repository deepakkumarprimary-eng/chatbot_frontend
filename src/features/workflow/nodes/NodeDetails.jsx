...
/* BFS upward — find nearest ancestor API node with apiType=condition */
function findUpstreamApiNode(nodeId, nodes, edges) {
  ...
  while (queue.length) {
    ...
    for (const edge of edges.filter((e) => e.target === current)) {
      ...
      if (!parent) continue;
      if (
        parent.data?.config?.nodeType === "api" &&
        parent.data?.config?.apiType === "condition"
      ) return parent;
...
function ConditionBranchEditor({
  edge, index, nodes, contextVariables, loadingVars,
  onConditionChange, onTargetChange, loopBackOptions,
}) {
      ...
      {loadingVars ? (
        ...
        <div className="row g-2 mb-3">
          <div className="col-md-4">
            ...
            <Form.Select size="sm" value={selectedVar} onChange={(e) => update({ variable: e.target.value, operator: "", value: "" })}>
              <option value="">— select variable —</option>
              {contextVariables.map((v) => <option key={v} value={v}>{v}</option>)}
            </Form.Select>
          </div>
          <div className="col-md-3">
            <Form.Label style={{ fontSize: 11.5, fontWeight: 600, color: "#475569" }}>Type</Form.Label>
            <Form.Select size="sm" value={selectedType} onChange={(e) => update({ varType: e.target.value, operator: "", value: "" })}>
              {TYPE_OPTIONS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </Form.Select>
          </div>
          <div className="col-md-3">
            <Form.Label style={{ fontSize: 11.5, fontWeight: 600, color: "#475569" }}>Operator</Form.Label>
            <Form.Select size="sm" value={selectedOp} disabled={!selectedVar} onChange={(e) => update({ operator: e.target.value })}>
          ...
          {!isBool && (
            <div className="col-md-2">
              ...
              <Form.Control size="sm"
                type={selectedType === "integer" || selectedType === "float" ? "number" : selectedType === "date" ? "date" : "text"}
                value={condValue} disabled={!selectedOp} placeholder="value"
...
const NodeDetails = ({ selectedNode, nodes, setNodes, edges, setEdges }) => {
  ...
  useEffect(() => {
    ApiConfigService.getAll().then(({ data }) => setApiList(data)).catch(() => {});
    WorkflowService.getAll().then(({ data }) => setWorkflowList(data)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    if (node.type === "decision") {
      ...
      upstream?.data?.config?.apiConfigId
  ...
  const fetchContextVariables = async (apiId) => {
    ...
    try {
      ...
      const { data } = await ApiConfigService.getById(apiId);
  ...
  const updateNode = (field, value) =>
    setNodes((nds) => nds.map((nd) =>
      nd.id === selectedNode.id ? { ...nd, data: { ...nd.data, [field]: value } } : nd));

  const updateNodeConfig = (key, value) =>
    setNodes((nds) => nds.map((nd) =>
      nd.id === selectedNode.id
        ? { ...nd, data: { ...nd.data, config: { ...nd.data?.config, [key]: value } } }
        : nd));
  ...
  const handleApiTypeChange = (newApiType) => {
    ...
    const alreadyHasDecision = outgoing.some(
      (e) => nodes.find((n) => n.id === e.target)?.type === "decision"
    );
  ...
  const handleConditionTargetChange = (edgeId, newTargetId) => {
    setEdges((eds) => eds.map((ed) =>
      ed.id === edgeId
        ? { ...ed, target: newTargetId, id: `${ed.source}->${newTargetId}-${Date.now()}` }
        : ed));
  };
              ...
              <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: "16px 20px", background: "#f8faff" }}>
                <div className="d-flex align-items-center justify-content-between mb-2">
                  <div>
                    ...
                    <span style={{ fontSize: 12, color: "#64748b", marginLeft: 8 }}>
                      Select one context variable to display from the API response
                    </span>
                  </div>
                ...
                {loadingVariables ? (
                  ...
                  <p style={{ fontSize: 13, color: "#94a3b8", margin: 0, fontStyle: "italic" }}>
                    No response mappings defined for this API.
                  </p>
                ) : (
                    ...
                    {contextVariables.map((varName) => {
                      ...
                      return (
                        <label key={varName} onClick={() => handleVariableSelect(varName)}
                          ...
                          <code style={{ fontSize: 13, background: "transparent", padding: 0, color: "inherit" }}>
                            {varName}
                          </code>
                        </label>
                      );
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, marginBottom: 3 }}>
                      Connected to API node: "{upstreamApiNode.data?.label}"
                    </div>
