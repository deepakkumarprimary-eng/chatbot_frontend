    ...
    <div className="container-fluid py-4" style={{ maxWidth: 1200 }}>
      ...
      {loading ? (
        ...
        <div className="text-center py-5 rounded border" style={{ background: "#f8f9fa", color: "#6c757d" }}>
          ...
          <div className="d-flex justify-content-center gap-2">
            <Button variant="outline-primary" onClick={() => navigate("/builder")}>🛠️ Create with Builder</Button>
          ...
          <Table hover className="mb-0 align-middle">
            <thead className="table-light">
              <tr>
                ...
                <th style={{ width: 110 }}>Transitions</th>
                <th>Created</th>
                <th>Updated</th>
            ...
            <tbody>
              {workflows.map((wf, index) => (
                <tr key={wf.id}>
                  ...
                  <td><Badge bg="primary" style={{ fontSize: 12 }}>{nodeCount(wf)}</Badge></td>
                  <td><Badge bg="secondary" style={{ fontSize: 12 }}>{wf.workflowJson?.transitions?.length ?? 0}</Badge></td>
                  <td style={{ fontSize: 13, color: "#6c757d" }}>{fmt(wf.createdAt)}</td>
                  <td style={{ fontSize: 13, color: "#6c757d" }}>{fmt(wf.updatedAt)}</td>
                  <td>
                    <div className="d-flex gap-1 flex-wrap">
                      <Button size="sm" variant="outline-secondary" onClick={() => setPreview(wf)}>View</Button>
                      <Button size="sm" variant="outline-info" onClick={() => navigate(`/builder/edit/${wf.id}`)}>Builder</Button>
                      <Button size="sm" variant="outline-primary" onClick={() => navigate(`/workflows/edit/${wf.id}`)}>Edit</Button>
                      <Button size="sm" variant="outline-danger" disabled={deletingId === wf.id}
                        onClick={() => handleDelete(wf.id, wf.name)}>
                        {deletingId === wf.id ? <Spinner animation="border" size="sm" /> : "Delete"}
                      </Button>
                    </div>
                  </td>
                </tr>
      ...
      <Modal show={!!preview} onHide={() => setPreview(null)} size="lg" centered>
        <Modal.Header closeButton className="workflow-header">
          <Modal.Title style={{ fontSize: 17, fontWeight: 700 }}>🗂️ {preview?.name}</Modal.Title>
        </Modal.Header>
                  ...
                  <div key={n.id} style={{ marginBottom: 8, padding: "8px 10px", borderRadius: 8, background: "#fff", border: "1px solid #e2e8f0", fontSize: 13 }}>
                    ...
                    <div style={{ color: "#64748b", fontSize: 11.5, marginTop: 2 }}>
                      <Badge bg={n.type === "decision" ? "warning" : n.type === "workflow" ? "success" : "primary"}
                        text={n.type === "decision" ? "dark" : undefined} style={{ fontSize: 10 }}>
                        {n.type}
                      </Badge>
                      {n.config?.nodeType && <span style={{ marginLeft: 6, color: "#94a3b8" }}>{n.config.nodeType}</span>}
                    </div>
                  </div>
                ))}
              </div>
              <div style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#6366f1", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>Raw JSON</p>
        ...
        <Modal.Footer style={{ borderTop: "1px solid #e5e7eb", padding: "12px 20px", gap: 8 }}>
          <Button variant="outline-info" onClick={() => { setPreview(null); navigate(`/builder/edit/${preview?.id}`); }}>Open in Builder</Button>
          <Button variant="outline-primary" onClick={() => { setPreview(null); navigate(`/workflows/edit/${preview?.id}`); }}>Edit JSON</Button>
